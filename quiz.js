...
function handleNewPost(event) {
  ...
  // Check if there is content and an image
  if (postContent && postImageFile) {
    // Create a new post object
    const newPost = {
      id: postsData.length + 1, // Generate a new ID
      author: "You", // Author is always "You" for new posts
      content: postContent,
      likes: 0,
      comments: [],
      image: URL.createObjectURL(postImageFile), // Create a URL for the image file
    };

    // Add the new post to the postsData array
    postsData.push(newPost);

    // Re-render the posts to include the new post
    renderPosts();

    // Clear the form inputs
    document.getElementById("postForm").reset();
  } else {
    alert("Please enter some content and select an image for your post.");
  }
}
